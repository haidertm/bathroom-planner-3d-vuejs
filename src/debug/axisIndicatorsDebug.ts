// src/debug/axisIndicatorsDebug.ts
...
type IndicatorDirection = 'vertical' | 'horizontal'

export class AxisIndicatorsDebug {
  private indicators: THREE.Object3D[] = []
  ...
  createAxisIndicators(
    ...
    // Calculate interior boundaries (same as getInteriorBoundaries function)
    const wallThickness = 5 // Matches WALL_SETTINGS.THICKNESS
    const interior: InteriorBounds = {
    ...
    let notch: NotchBounds | null = null
    ...
    const wallHeight = 250 // Matches WALL_SETTINGS.HEIGHT
   ...
   */
  private createXAxisIndicators(
    scene: THREE.Scene,
    interior: InteriorBounds,
    notch: NotchBounds | null,
    roomHeight: number,
    wallOffset: number,
   ...
   */
  private createTextSprite(text: string, color: string, size: 'small' | 'normal' = 'normal'): THREE.Sprite {
   ...
   */
  clearIndicators(_scene: THREE.Scene): void {
    this.indicators.forEach(indicator => {
      if (indicator.parent) {
        indicator.parent.remove(indicator)
      }
      // Dispose of materials and geometries
      if ('material' in indicator) {
        const material = indicator.material as THREE.Material & { map?: THREE.Texture }
        if (material.map) material.map.dispose()
        material.dispose()
      }
      if ('geometry' in indicator) {
        (indicator.geometry as THREE.BufferGeometry).dispose()
      }
    })
    this.indicators = []
  }
   ...
   */
  updateIndicators(scene: THREE.Scene, roomWidth: number, roomHeight: number): void {
    if (this.enabled) {
      this.createAxisIndicators(scene, roomWidth, roomHeight)
    }
  }
