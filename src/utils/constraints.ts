...
export const checkCollision = (
  ...
  if (obj1Wall === 'east' || obj1Wall === 'west' || obj1Wall === 'notch-east') {
    ...
    console.log('🔄 Computing AABB for free-rotation object 1:', {
      sku: item1?.sku,
      rotation: item1?.rotation?.toFixed(2),
      baseDims: `${obj1BaseWidth.toFixed(1)} × ${obj1BaseDepth.toFixed(1)}`,
      aabbDims: `${obj1Width.toFixed(1)} × ${obj1Depth.toFixed(1)}`
    });
  } else {
  ...
  if (obj2Wall === 'east' || obj2Wall === 'west' || obj2Wall === 'notch-east') {
    ...
    console.log('🔄 Computing AABB for free-rotation object 2:', {
      sku: item2?.sku,
      rotation: item2?.rotation?.toFixed(2),
      baseDims: `${obj2BaseWidth.toFixed(1)} × ${obj2BaseDepth.toFixed(1)}`,
      aabbDims: `${obj2Width.toFixed(1)} × ${obj2Depth.toFixed(1)}`
    });
  } else {
  ...
  // Larger buffers to prevent visual overlaps
  const horizontalBuffer = 15; // 15cm horizontal buffer (30cm total gap when near)
  const verticalBuffer = 5;    // 5cm vertical buffer (10cm total gap when stacked)
  ...
  switch (nearestWall) {
    case 'north':
      ...
      if (wouldExtendWest || wouldExtendEast) {
        ...
        console.log(`🔧 :::: X constrained due to room bounds (North): ${position.x.toFixed(1)} → ${constrainedPosition.x.toFixed(1)}`);
      } else {
    ...
    case 'west':
      ...
      if (wouldExtendNorthWest || wouldExtendSouthWest) {
        ...
        console.log(`🔧 :::: Z constrained due to room bounds (West): ${position.z.toFixed(1)} → ${constrainedPosition.z.toFixed(1)}`);
      } else {
        constrainedPosition.z = position.z;
        console.log(`🎯 :::: Z preserved: ${position.z.toFixed(1)} (no room boundary conflict)`);
      }
...
export const findFreeCornerPosition = (
  ...
  for (const corner of corners) {
    ...
    // Check wall collision - object must fit within room boundaries
...
export const constrainAllObjectsToRoom = (
  ...
  for (const { item, targetWall, position, movementConfig } of itemsWithMetadata) {
    ...
    if (hasCollision) {
      console.log(`⚠️ Collision detected for ${item.type} (id: ${item.id}) after resize. Attempting to resolve...`);
      ...
      for (const offset of offsets) {
        ...
        if (!wouldCollideWithExisting(
          ...
          resolved = true;
          console.log(`✅ Collision resolved for ${item.type} with offset ${offset} on ${axis} axis`);
          break;
        }
      }
      ...
      if (!resolved) {
        ...
        for (const wall of candidateWalls) {
          ...
          if (!wouldCollideWithExisting(
            ...
            constrainedRotation = reConstrained.rotation;
            resolved = true;
            console.log(`✅ Moved ${item.type} to ${wall} wall to avoid collision.`);
            break;
          }
...
/**
 * Validate if an object can fit in the room based on its dimensions
...
/**
 * Calculate available space for variant swapping at current item position
 * Returns the maximum width a variant can have at the current position
 * considering walls and other objects
 */
